<template>
  <v-container grid-list-xl>
    ...
    <v-layout class="hidden-sm-and-down" row wrap style="margin-top:-5%;" md10 >
        <v-flex md1 style="margin-right:5%;" v-for="(specification, index) in specificationsList" :key="index" >
    ...
    <v-layout class="hidden-md-and-up" row wrap style="margin-top:-20%;margin-left:-20%">
        <v-flex xs2 v-for="(specification, index) in specificationsList" :key="index" >
            <v-btn round  @click="filterBySpecification(specification.id), gammas = [true,true,true]" flat color="rgb(14, 49, 138)" class="">
                <v-icon large>{{specification.icon}}</v-icon>
            </v-btn>
        </v-flex>
